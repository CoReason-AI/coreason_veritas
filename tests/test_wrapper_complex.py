...
def test_threaded_sync_execution(key_pair):
...
@pytest.mark.asyncio
async def test_mixed_nesting_sync_calls_async(key_pair):
    """Test Sync function calling Async governed function (needs special handling by caller, but verifies wrapper compatibility)."""
    private_key, public_key_pem = key_pair
    payload = {"d": 1}
    sig = sign_payload(payload, private_key)
    ...
    with patch.dict(os.environ, {"COREASON_VERITAS_PUBLIC_KEY": public_key_pem}):
        with patch("coreason_veritas.auditor.trace.get_tracer") as mock_get_tracer:
            ...
            @governed_execution(asset_id_arg="spec", signature_arg="sig", user_id_arg="user")
            async def inner_async(spec, sig, user):
                assert is_anchor_active()
                return "inner"

            @governed_execution(asset_id_arg="spec", signature_arg="sig", user_id_arg="user")
            def outer_sync(spec, sig, user):
                assert is_anchor_active()
...
def test_evil_str_attribute(key_pair):
...
@pytest.mark.asyncio
async def test_governed_async_generator(key_pair):
    ...
    with patch.dict(os.environ, {"COREASON_VERITAS_PUBLIC_KEY": public_key_pem}):
        with patch("coreason_veritas.auditor.trace.get_tracer") as mock_get_tracer:
            ...
            @governed_execution(asset_id_arg="spec", signature_arg="sig", user_id_arg="user")
            async def my_agen(spec, sig, user):
                yield 1
                if not is_anchor_active():
