...
def sign_payload(payload, p_key=private_key):
    canonical = jcs.canonicalize(payload)
    signature = p_key.sign(
        canonical, padding.PSS(mgf=padding.MGF1(hashes.SHA256()), salt_length=padding.PSS.MAX_LENGTH), hashes.SHA256()
    )
...
def test_verify_asset_missing_timestamp():
...
def test_verify_asset_timestamp_too_old():
...
def test_verify_asset_timestamp_future():
...
def test_verify_asset_wrong_signature():
    ...
    sig = sign_payload(payload, p_key=other_private_key)

    with pytest.raises(AssetTamperedError, match="Signature verification failed"):
        validator.verify_asset(payload, sig)


def test_verify_asset_malformed_signature_string():
...
def test_verify_asset_large_payload():
...
def test_verify_asset_nested_json():
...
def test_verify_asset_unicode_characters():
